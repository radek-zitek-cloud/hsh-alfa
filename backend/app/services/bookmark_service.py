...
class BookmarkService:
    ...
    async def list_bookmarks(
        self, user_id: int, category: Optional[str] = None, sort_by: Optional[str] = None
    ) -> List[Bookmark]:
        """
        ...
        logger.debug(
            "Listed bookmarks from database",
            extra={
    ...
    async def create_bookmark(self, bookmark_data: BookmarkCreate, user_id: int) -> Bookmark:
        ...
        if not favicon_url:
            try:
                logger.debug(
                    "Attempting to fetch favicon",
                    extra={"operation": "bookmark_favicon_fetch", "url": bookmark_data.url},
                )
                favicon_url = await fetch_favicon(bookmark_data.url)
                if favicon_url:
                    logger.info(
                        "Favicon fetched automatically",
                        extra={
    ...
    async def update_bookmark(
        self, bookmark_id: int, bookmark_data: BookmarkUpdate, user_id: int
    ) -> Optional[Bookmark]:
        """
        ...
        if "url" in update_data and "favicon" not in update_data:
            try:
                ...
                logger.debug(
                    "Attempting to fetch favicon for updated URL",
                    extra={"operation": "bookmark_favicon_fetch", "url": new_url},
                )
                favicon_url = await fetch_favicon(new_url)
                if favicon_url:
                    update_data["favicon"] = favicon_url
                    logger.info(
                        "Favicon fetched for updated URL",
                        extra={
                            "operation": "bookmark_favicon_fetched",
                            "url": new_url,
                            "favicon_url": favicon_url,
                    ...
                    logger.warning(
                        "Could not fetch favicon for updated URL",
                        extra={
                            "operation": "bookmark_favicon_fetch_failed",
                            "url": new_url,
                            "reason": "no_favicon_found",
                ...
                logger.error(
                    "Error fetching favicon for updated URL",
                    extra={
                        "operation": "bookmark_favicon_fetch_error",
