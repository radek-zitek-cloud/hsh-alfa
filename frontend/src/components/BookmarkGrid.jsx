...
const BookmarkCard = ({ bookmark, onDelete, onEdit }) => {
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error) => {
      ...
      onDelete?.('Failed to delete bookmark', 'error')
    },
    onSettled: () => {
      setIsDeleting(false)
    },
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
      {bookmarks.map((bookmark) => (
        <BookmarkCard
          key={bookmark.id}
          bookmark={bookmark}
          onDelete={(message, type = 'success') => onToast?.(message, type)}
          onEdit={(message) => onToast?.(message, 'success')}
        />
      ))}
    </div>
  )
